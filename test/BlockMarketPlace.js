...
describe("BlockMarketPlace Test Suite", () => {
  ...
  describe("Buy NFT", () => {
    it("Should successfully buy NFT with ETH", async () => {
      ...
      const expectedSellerAmount = (price * 97n) / 100n;
      const expectedMarketplaceAmount = (price * 3n) / 100n;
      ...
      expect(addr1BalanceAfter - addr1BalanceBefore).to.eq(
    ...
    it("Should successfully buy NFT with ERC20", async () => {
  ...
  describe("Making Offers", () => {
    it("Should successfully make ETH offer", async () => {
      const { marketplace, blocknft, addr1, addr2 } = await loadFixture(
        deployBlockMarketPlace
      );
    ...
    it("Should successfully make ERC20 offer", async () => {
      const { marketplace, blocknft, BlockToken, owner_, addr1, addr2 } =
        await loadFixture(deployBlockMarketPlace);
    ...
    it("Should revert if offer is below minimum", async () => {
      const { marketplace, blocknft, addr1, addr2 } = await loadFixture(
        deployBlockMarketPlace
      );
      ...
      let tokenId = 1;
      let price = ethers.parseEther("1");
      let lowOffer = ethers.parseEther("0.3"); // Below minOffer of 0.5
